...
from models import db, Admin, LostItem, FoundItem, Category, Location, Review, User, MatchedItem, Claim, Settings
from forms import LoginForm, ReportLostItemForm, ReportFoundItemForm, RegistrationForm, ResetPasswordForm, ResetPasswordRequestForm, UserPortalForm, VerifyCodeForm, ProofOfOwnershipForm, SettingsForm
...
def validate_ownership(proof):
    if not proof:
        return "Proof of ownership is required"
    if len(proof) < 10:  
        return "Proof must be at least 10 characters long"
    return None  
...
def report():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('report.html', categories=categories, locations=locations, form=form, matches=matches)
...
app.route('/report_found_item', methods=['GET', 'POST'])
@login_required
def report_found_item():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash("Invalid category or location selection.", "danger")
            return render_template('report_found.html', categories=categories, locations=locations, form=form, matches=matches)
        ...
        if date_found:
            ...
            flash("Please provide a valid date.", "danger")
            return render_template('found.html', categories=categories, locations=locations, form=form, matches=matches)
    ...
    return render_template('report_found_item.html', categories=categories, locations=locations, form=form, matches=matches)
...
def claim_item(item_id, item_type):
    ...
    if form.validate_on_submit():
        ...
        if validate_ownership(proof_details, item):  
            item.status = 'Claimed'
            item.claimed_by = current_user.id  
            item.ownership_proof = uploaded_file_path  
            db.session.commit()
            ...
            send_claim_notification(current_user.email, item)

            flash("Item claimed successfully! A notification has been sent to your email.", "success")
            return redirect(url_for('home'))
        else:
            flash("Unable to verify ownership. Please provide accurate details.", "danger")
...
def settings():
    ...
    if request.method == 'POST':
        ...
        settings.notifications_enabled = bool(request.form.get('notifications_enabled'))
